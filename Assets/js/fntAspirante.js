...
const showPersonalDescription = async (data) => {
  const perfilLaboralAspirante = document.querySelector(
    "#perfil-laboral-aspirante"
  );
...
const insertPuestoInteresAspirante = async () => {
  if (document.getElementById("txtPuesto").value.trim() === "") {
    sweetAlert(
      "Campos obligatorio",
      "Por favor, seleccione un puesto de interés !!",
      "info"
    );
  } else {
    const formData = formDataElement(formPuestoInteres);
...
const seleccionarIdioma = async () => {
  ...
  document
    .querySelectorAll("#txtListIdioma .opciones-idiomas")
    .forEach((option) => {
      if (
        option.selected &&
        formData.get("txtNivelIdioma") !== null &&
        option.value !== "" &&
        option.disabled === false
      ) {
        existeIdioma = arrObjIdiomas.some(
        ...
        arrObjIdiomas.forEach(
          (idioma) => (strNombreIdiomas += `${idioma.nombre}-`)
        );
...
document
  .querySelector("#agregar_idioma")
  .addEventListener("click", seleccionarIdioma);
...
const insertNewIdioma = async () => {
  ...
  const formData = formDataElement(formIdioma);
  if (
    formData.get("txtIdioma").trim() === null ||
    formData.get("txtNivelIdioma") === null
  ) {
    sweetAlert(
      "Campos obligatorios",
      "Debe de introducir un valor en el campo de idioma y asignarle una puntuación.",
      "warning"
    );
  } else {
    try {
  ...
  try {
    ...
    if (status && data !== "no") {
      if (document.querySelector("#data-idIdiomas i")) {
        document
          .querySelector("#data-idIdiomas i")
          .classList.replace("la-plus", "la-pen");
      }
...
const seleccionarHabilidad = async () => {
  ...
  document
    .querySelectorAll("#txtListHabilidad .opciones-habilidad")
    .forEach((option) => {
      if (
        option.selected &&
        formData.get("txtNivelHabilidades") !== null &&
        option.value !== "" &&
        option.disabled === false
      ) {
        existeHabilidad = arrObjHabilidad.some(
          (item) => item.nombre === option.value
        );
        if (existeHabilidad) {
          swal("Error", "Aspirante ya tiene asociado este idioma.", "warning");
        ...
        arrObjHabilidad.forEach(
          (idioma) => (strNombrehabilidad += `${idioma.nombre}-`)
        );
        ...
        document.querySelector("#nivelHabilidad").value = formData.get(
          "txtNivelHabilidades"
        );
...
document
  .querySelector("#agregar_habilidad")
  .addEventListener("click", seleccionarHabilidad);
  ...
  try {
    ...
    if (status && data !== "no") {
      if (document.querySelector("#data-idHabilidades i")) {
        document
          .querySelector("#data-idHabilidades i")
          .classList.replace("la-plus", "la-pen");
      }
...
const loadDataEditEducation = async (e) => {
  ...
  try {
    ...
    if (status) {
      ...
      document.querySelector("#input-institucion").value =
...
const loadDataEditExperience = async (e) => {
  ...
  try {
    ...
    if (status) {
      ...
      document.querySelector("#txtPuestoDesempeñado").value =
        data.nombrePuestoDesempeño;
...
const validateFormPersonalInformation = async (e) => {
  ...
  if (
    ...
    sweetAlert(
      "Campos obligatorios!",
      "Se debe  rellenar todos lo campos. Todos son obligatorios!",
      "error"
    );
  } else {
...
if (document.getElementById("grupo-otra_habilidad")) {
  document
    .getElementById("grupo-otra_habilidad")
...
const validateFormEstudios = (e) => {
  ...
  if (
    ...
    tituloObtenido.value.trim() === "" ||
    ciudad.value.trim() === "" ||
    gradoEstudio.value.trim() === "" ||
    sector.value.trim() === "" ||
    anioInicio.value.trim() === "" ||
    mesInicio.value.trim() === "" ||
    anioFin.value.trim() === "" ||
    mesFin.value.trim() === ""
  ) {
    ...
    sweetAlert(
      "Datos incorrectos",
      "Apreciado usuario el año de finalización de sus estudios no puede ser menor al año de inicio.",
      "warning"
    );
  } else if (
    parseInt(anioInicio.value) === parseInt(anioFin.value) &&
    parseInt(mesInicio.value) > parseInt(mesFin.value)
  ) {
    sweetAlert(
      "Datos incorrectos",
      "Apreciado usuario el mes de finalización de sus estudios no puede ser menor al mes de inicio.",
      "warning"
    );
  } else {
    insertEstudio();
...
const validateFormExperienciaLaboral = (e) => {
  ...
  if (
    empresa.value.trim() === "" ||
    sectorLaboro.value.trim() === "" ||
    ciudad.value.trim() === "" ||
    tipoExperiencia.value.trim() === "" ||
    puestoDesempeño.value.trim() === "" ||
    anioInicio.value.trim() === "" ||
    mesInicio.value.trim() === "" ||
    anioFin.value.trim() === "" ||
    mesFin.value.trim() === "" ||
    funcionDesempeño.value === ""
  ) {
    sweetAlert("Error", "Todos los campos son obligatorios!!", "error");
  } else if (parseInt(anioFin.value) < parseInt(anioInicio.value)) {
    sweetAlert(
      "Datos incorrectos",
      "Apreciado usuario el año de finalización laboral no puede ser menor al año de inicio.",
      "warning"
    );
  } else if (
    parseInt(anioInicio.value) === parseInt(anioFin.value) &&
    ...
    sweetAlert(
      "Datos incorrectos",
      "Apreciado usuario el mes de finalización laboral no puede ser menor al mes de inicio.",
      "warning"
    );
  } else {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (document.querySelector("#edit-Aspirante")) {
    document
      .querySelector("#edit-Aspirante")
      .addEventListener("click", (e) => loadDataEditPersonalDescription(e));
  }
  ...
  if (document.querySelector(".estudio-aspirante")) {
    document
      .querySelectorAll(".estudio-aspirante")
      .forEach((element) =>
        element.addEventListener("click", (e) => loadDataEditEducation(e))
      );
  }

  if (document.querySelector(".estudio-experiencia")) {
    document
      .querySelectorAll(".estudio-experiencia")
      .forEach((element) =>
        element.addEventListener("click", (e) => loadDataEditExperience(e))
      );
  }
});
 ...
 */
formDescripcionPersonal.addEventListener(
  "submit",
...
document
  .querySelector("#boton_add_puesto")
  .addEventListener("click", insertPuestoInteres);

document
  .querySelector("#btn-puesto-interes")
  .addEventListener("click", validateFormPuestoInteres);
...
if (document.querySelector("#data-idExperiencia")) {
  document
    .querySelector("#data-idExperiencia")
    .addEventListener("click", () => initTextEditorTinymce("funciones"));
}
